...
class SoccerRobot(ABC):
    ...
    """
        ...
        # print(self.supervisorData)

    def getRedDefenderLeft(self) -> list:
        return [self.supervisorData[15], self.supervisorData[16]]

    def getRedDefenderRight(self) -> list:
        return [self.supervisorData[18], self.supervisorData[19]]
    ...
    """
        if self.currentlyMoving and self.currentlyMoving.name == "forwardsSprint" and self.currentlyMoving.getTime() == 1360:  # we reached the end of forward.motion
            self.currentlyMoving.stop()
            self.currentlyMoving = False
            print("interrupt forwards sprit")
