...
class ForwardLeft(SoccerRobot):
                ...
                if self.checkGoal() == 1:
                    ...
                    if self.isNewMotionValid(decidedMotion):
                        boolean = self.currentlyMoving and \
                                  (
                                              self.currentlyMoving.name == self.motions.forwards.name and decidedMotion.name != self.motions.forwards.name)
                        if boolean:
                ...
                # Check the oponent has ball priority.
                    ...
                    if flag1 == 0:
                        if ballOwner == 'BLUE_FW_L' or ballOwner[0] == 'R':
                            ...
                            if rightForward[0] > -4.47 and rightForward[0] < 4.44 and rightForward[1] > 0 and \
                                    rightForward[1] < 1.5:
                                goto_Coordinate[0] = rightForward[0] - 1.5
                                goto_Coordinate[1] = rightForward[0] - 1
                                goto_Coordinate[2] = 0.343
                                decidedMotion, useless_flag = self.decideMotion(goto_Coordinate, selfCoordinate,
                                                                                post_coordinate)
                                if self.isNewMotionValid(decidedMotion):
                                    boolean = self.currentlyMoving and \
                                              (
                            ...
                            elif rightForward[0] > -4.47 and rightForward[0] <= 0.268 and rightForward[1] < -1.5 and \
                                    rightForward[1] > -2.96:
                                goto_Coordinate[0] = rightForward[0] + 1.5
                                goto_Coordinate[1] = rightForward[0] - 1
                                goto_Coordinate[2] = 0.343
                            ...
                            if ballCoordinate[0] <= 2.52 or (ballCoordinate[1] < -2.4 and ballCoordinate[1] > -2.98):
                                decidedMotion, flag1 = self.decideMotion(ballCoordinate, selfCoordinate,
                                                                         post_coordinate)
                                if self.isNewMotionValid(decidedMotion):
                                    boolean = self.currentlyMoving and \
                                              (
                        ...
                        decidedMotion, flag1 = self.turn_to_goal_post(post_coordinate, selfCoordinate, rightForward,
                                                                      redForward)
                        if count_0 >= 2:
    ...
    def decideMotion(self, ballCoordinate, selfCoordinate, post_coordinate):
        ...
        return self.motions.forwards, 0

    def turn_to_goal_post(self, post_coordinate, selfCoordinate, rightForward, redForward):

        self.clearMotionQueue()
        robotHeadingAngle = self.getRollPitchYaw()[2]
        turningAngle = Functions.calculateTurningAngleAccordingToRobotHeading(post_coordinate, selfCoordinate,
                                                                              robotHeadingAngle)
